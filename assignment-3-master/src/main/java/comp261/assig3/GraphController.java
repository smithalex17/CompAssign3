...
public class GraphController {
    ...
    public void handleAugmentationPaths(ActionEvent event) {
        ...
        if (aPaths != null) {
            ...
            for(Pair p: aPaths){
                ArrayList<Node> nL = (ArrayList) p.getKey();
                for(Node n: nL){
    ...
    public void handleMinCut(ActionEvent event) {
        ...
        if (minCutwithSets != null) {
            ...
            HashSet<Node> setT = new HashSet<Node>();
            Pair p = minCutwithSets.getKey();

            setS = (HashSet<Node>) p.getKey();
            setT = (HashSet<Node>) p.getValue();
            for(Node n : setS){
                highlightNodes.add(n);
            }
            for(Node n : setT){
